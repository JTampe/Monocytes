---
title: "2023-12-31_Data Analysis of Monocyte Control Samples - Final Version"
output:
  html_document: 
    code_folding: show
    fig_caption: yes
...
#setwd("~/Documents/Projects/monocyte Characterization/Data/231231_Controls_Final/")
setwd("~/Library/CloudStorage/OneDrive-LundUniversity/Dokument/Projects/Monocyte Characterization/Data/231231_Controls_Final/")
getwd()
rm(list=ls())
...
## LinReg Plot - sex
also saves the plot 
```{r Function - LinRegresseion separated by ssex}
...
```{r Function - Weighted Correlation}
...
```
Once loaded, label and bring data in the right format.

The data loaded had the following variables & Frequencies:
...
We extract the individual geneses measured in our runs: 
Following genes are evaluated for the dCT value:
...
```
Following patients are evaluated for the dCT value:
...
```{r Sort and group data, echo=FALSE}
info_rows = t(data.frame(lapply( dataTRIM$Sample, function(x){ 
    ret = unlist( stringr::str_split( x, "[_\\s\\.]")); 
    if ( length(ret) < 4){
        ret = c(ret, rep(0, 4-length(ret) ))
    } 
    ret  } ) ))
...
## remove Outliers
```{r Outliners, eval=FALSE, include=FALSE}
...
It's easier to make the imputation or less columnes/variables
```{r remove info}
# remove test better!
...
## Failed Genes
List the failed genes as Quality Control
```{r failed genes}
fails <- dataAll %>%
...
```{r effect of variabels, eval=FALSE, include=FALSE}
...
```{r Check the imputation, eval=FALSE, include=FALSE}
...
patients
```
The total amount of patients are evaluating :
```{r No Patients data, echo=FALSE}
...
```
Also the metadata is loaded and Age groups assessed.
Patients between 28 to 65 are considered Adult, individuals above & including 65 are considered Older.
...
We then also use the base functions of R to create mean and confidence intervals (CI) for the CT values. NAs are skipped
--> not needed when Technical and biological replicates are seperated as they are no "replicates so no mean /sd can be calculated)
```{r Mean and SD}
...
```
## Age Summary Matrix
```{r Age Summary Matrix, echo=FALSE}
...
```
## Sex Summary Matrix
```{r Sex Summary Matrix, echo=FALSE}
...
## Gene data
Following files have been generated for each gene in the Folder Data_Genes separated:
```{r Split data - Gene, echo=FALSE}
...
```
## Subtype data
Following files have been generated for each subtype in the Folder Data_Subtypes separated:
```{r Split data - Subtype, echo=FALSE}
...
## Male/Female data
Following files have been generated for each sex in the Folder Data_Sex separated:
```{r Split data - Sex, echo=FALSE}
...
```
## Age & Sex Group data
Following files have been generated for each study group in the Folder Data_Group separated:
```{r Split data - Group, echo=FALSE}
...
print(group_list)
```
In the groups we have the following Sample sizes:
Middle aged Females:
```{r}
...
```
# *Statistics*
Pearson's correlation coefficient assumes that the variables have a linear relationship and are normally distributed. If these assumptions hold true for your data, using Pearson's correlation can be a valid alternative. However, if your data violates these assumptions or if you suspect a nonlinear relationship between the variables, Spearman's correlation is more appropriate.
...
```{r Linear Regression (FACS) - Pearson, echo=FALSE}
...
```{r Linear Regression - Pearson w/o Outliners}
...
```{r Gene vs Age (split by Sex) w/o Outliners}
createFolder("Statistics_LinReg_woOutliers")
...
```{r Plot linear regressions, eval=FALSE, include=FALSE}
