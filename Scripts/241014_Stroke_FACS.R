...
Matched_TP0_FACS <- c("Ctr01", "Ctr02", "Ctr08", "Ctr09", "Ctr11", "Ctr14", "Ctr16", "Ctr17", "Ctr18", "Ctr25", "Ctr27", "Ctr29", "Ctr31","Ctr41", "Ctr43")
...
ggsave(filename = paste(output_location, "/Dotplots_Gene_summary/Exp_by_TP.png", sep=""))
...
ggsave(filename = paste(output_location, "/Dotplots_Gene_summary/Exp_by_Suptype.png", sep=""))
...
ggsave(filename = paste(output_location, "/Dotplots_Gene_summary/Exp_by_TP_grey.png", sep=""))
...
ggsave(filename = paste(output_location, "/Dotplots_Gene_summary/Exp_by_Suptype_grey.png", sep=""))

## LinReg FACS outliers-----------------------------------------
createFolder("Statistics_LinReg_FACS")
...
output_folder <- "Statistics_LinReg_FACS/Plots"
...
# Save results to CSV
write.csv(Age_correlation_FACS, "Statistics_LinReg_FACS/Age_correlation_FACS.csv", row.names = FALSE)
write.csv(outliers_FACS, "Statistics_LinReg_FACS/outliers_FACS.csv", row.names = FALSE)

# *1WAY ANOVA* FACS----------------------------------------------------------------------------------------------------


automate_ANOVA("ANOVA_FACS_results", "ANOVAwilcox_FACS exOutliers", # Folders
               "ANOVA_FACS", # results name
               "n", # plots y / n
               FACSdata, # Dataset
               FACSdata[colnames(FACSdata)[4:16]]) # Loop
...
check_sample_counts(data_rE_mean)

#### Wilcox test for rE of Genes --------------------------------------------------
folder <- "Wilcox_rE_exOutliers"
...
for (i in 1:length(unique(data_rE_mean$Gene))) {
    gene <- unique(data_rE_mean$Gene)[i]
    df_gene <- filter(data_rE_mean, Gene == gene)
...
i <- 1
for (i in 1:length(unique(data_rE_mean$Gene))) {
    gene <- unique(data_rE_mean$Gene)[i]  # Use unique to avoid duplicates
    df_gene <- filter(data_rE_mean, Gene == gene)
