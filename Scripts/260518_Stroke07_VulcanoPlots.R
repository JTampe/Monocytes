...
subpops <- sort(unique(LinReg_NIHSS_Ratio_capZ$Subpopulation))
timepoints <- sort(unique(LinReg_NIHSS_Ratio_capZ$Timepoint))


for (sp in subpops) {
    for (tp in timepoints) {
        
        df_check <- LinReg_NIHSS_Ratio_capZ %>%
            filter(
                Subpopulation == sp,
                Timepoint == tp,
                Subgroup == "All",
        ...
        volcano_plot_linreg(
            results_df    = LinReg_NIHSS_Ratio_capZ,
            subpopulation = sp,
            timepoint     = tp,
            p_cutoff      = 0.05,
            coef_cutoff   = 0,
            ...
            plot_title    = paste(
                ...
                "Monocytes (", tp, "): NIHSS Ratio ~ Gene expression",
                sep = ""
            ),
            output_file   = paste0(
                "LinReg_Results_capZ/Volcano_",
                sp, "_", tp, "_NIHSS_Ratio.png"
            )
        )
    }
}
