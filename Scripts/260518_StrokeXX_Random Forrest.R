...
result <- run_rf_analysis(
    data = rf_data,
    features = c("Subpopulation", "Age", "Sex", "NIHSS", "mRS", "Barthel", "MoCA", "HADS_Anxiety", "HADS_Depression", 
                 as.vector(genes_tp2)),
    target = "NIHSS_Diff",
    prefix = "NIHSS_Diff"
)
...
result_important <- run_rf_analysis(
    ...
    features = c("Subpopulation", "Age", "Sex", "NIHSS", "mRS", "Barthel", "MoCA", "HADS_Anxiety", "HADS_Depression", 
                 as.vector(genes_tp2)),
    target = "NIHSS_Diff",
    prefix = "NIHSS_Diff_intermediate"
)
# nonclassical
result_important <- run_rf_analysis(
    ...
    features = c("Subpopulation", "Age", "Sex", "NIHSS", "mRS", "Barthel", "MoCA", "HADS_Anxiety", "HADS_Depression", 
                 as.vector(genes_tp2)),
    target = "NIHSS_Diff",
    prefix = "NIHSS_Diff_nonclassical"
)
...
result_important <- run_rf_analysis(
    ...
    features = "NIHSS",
    target = "NIHSS_Diff",
    prefix = "NIHSS_Diff_NIHSS"
)


### for NIHSS_End instead of Diff ----------------------------------
result_End <- run_rf_analysis(
    data = matrix_meanCapZ %>% filter(Timepoint == "TP2"),
    features = c("Subpopulation", "Age", "Sex", "NIHSS", "mRS", "Barthel", "MoCA", "HADS_Anxiety", "HADS_Depression", as.vector(genes_tp2)),
    target = "NIHSS_End",
    prefix = "NIHSS_End"
)
