...
Age_correlation_FACS <- data.frame(Cells = character(), Timepoint = character(), Focus = character(), N = numeric(), 
...
for (i in 4:16) {
    ...
    rows_all$Focus <- "All"  # Add the Sex label for all samples
    ...
    if (nrow(FACSdata_male) > 0) {
        ...
        rows_male$Focus <- "Male"  # Add the Sex label for male samples
    ...
    if (nrow(FACSdata_female) > 0) {
        ...
        rows_female$Focus <- "Female"  # Add the Sex label for female samples
    ...
    if (nrow(FACSdata_Minor) > 0) {
        rows_Minor <- calculate_linreg_by_timepoint(FACSdata_Minor, cells_colname)
        rows_Minor$Focus <- "Minor"  # Add the Category label for Minor samples
        Age_correlation_FACS <- rbind(Age_correlation_FACS, rows_Minor)
        
        # Plot regression for "Minor" samples with custom colors, size, and theme
        p_Minor <- ggplot(FACSdata_Minor, aes(x = Age, y = .data[[cells_colname]], color = Timepoint)) +
            geom_point(size = 2) +
            geom_smooth(method = "lm", aes(color = Timepoint), se = FALSE, size = 1) +  # Regression line in corresponding color
            ggtitle(paste("Linear Regression - Minor (", cells_colname, ")", sep = "")) +
            xlab("Age") +
            ylab(cells_colname) +
            theme(text = element_text(size = 14)) +
            theme(
        ...
        # Add confidence interval only if p-value is <= 0.05
        for (timepoint in unique(FACSdata_Minor$Timepoint)) {
            if (rows_Minor$p.value[rows_Minor$Timepoint == timepoint] <= 0.05) {
                p_Minor <- p_Minor + geom_smooth(data = subset(FACSdata_Minor, Timepoint == timepoint), 
                                                 method = "lm", 
                                                 aes(fill = Timepoint), 
                                                 se = TRUE, 
                                                 alpha = 0.25, 
        ...
        # Save the plot for "Minor" samples
        ggsave(filename = file.path(output_folder, paste0("LinReg_Minor_", cells_colname, ".png")), plot = p_Minor)
    }
    
    ### Moderate ---------
    FACSdata_Moderate <- FACSdata[FACSdata$Category == "MODERATE", ]
    if (nrow(FACSdata_Moderate) > 0) {
        rows_Moderate <- calculate_linreg_by_timepoint(FACSdata_Moderate, cells_colname)
        rows_Moderate$Focus <- "Moderate"  # Add the Category label for Moderate samples
        Age_correlation_FACS <- rbind(Age_correlation_FACS, rows_Moderate)
        
        # Plot regression for "Moderate" samples with custom colors, size, and theme
        p_Moderate <- ggplot(FACSdata_Moderate, aes(x = Age, y = .data[[cells_colname]], color = Timepoint)) +
            geom_point(size = 2) +
            geom_smooth(method = "lm", aes(color = Timepoint), se = FALSE, size = 1) +  # Regression line in corresponding color
            ggtitle(paste("Linear Regression - Moderate (", cells_colname, ")", sep = "")) +
            xlab("Age") +
            ylab(cells_colname) +
            theme(text = element_text(size = 14)) +
            theme(
        ...
        # Add confidence interval only if p-value is <= 0.05
        for (timepoint in unique(FACSdata_Moderate$Timepoint)) {
            if (rows_Moderate$p.value[rows_Moderate$Timepoint == timepoint] <= 0.05) {
                p_Moderate <- p_Moderate + geom_smooth(data = subset(FACSdata_Moderate, Timepoint == timepoint), 
                                                       method = "lm", 
                                                       aes(fill = Timepoint), 
                                                       se = TRUE, 
                                                       alpha = 0.25, 
        ...
        # Save the plot for "Minor" samples
        ggsave(filename = file.path(output_folder, paste0("LinReg_Moderate_", cells_colname, ".png")), plot = p_Moderate)
    }
        ### Good recovery -------
        FACSdata_Good <- FACSdata[FACSdata$Recovery == "Good", ]
        FACSdata_Good  <- FACSdata_Good [!is.na(FACSdata_Good$Age), ]
        if (nrow(FACSdata_Good) > 0) {
            rows_Good <- calculate_linreg_by_timepoint(FACSdata_Good, cells_colname)
            rows_Good$Focus <- "Good"  # Add the Recovery label for Good samples
            Age_correlation_FACS <- rbind(Age_correlation_FACS, rows_Good)
            
            # Plot regression for "Good" samples with custom colors, size, and theme
            p_Good <- ggplot(FACSdata_Good, aes(x = Age, y = .data[[cells_colname]], color = Timepoint)) +
                geom_point(size = 2) +
                geom_smooth(method = "lm", aes(color = Timepoint), se = FALSE, size = 1) +  # Regression line in corresponding color
                ggtitle(paste("Linear Regression - Good (", cells_colname, ")", sep = "")) +
                xlab("Age") +
                ylab(cells_colname) +
                theme(text = element_text(size = 14)) +
                theme(
            ...
            # Add confidence interval only if p-value is <= 0.05
            for (timepoint in unique(FACSdata_Good$Timepoint)) {
                if (rows_Good$p.value[rows_Good$Timepoint == timepoint] <= 0.05) {
                    p_Good <- p_Good + geom_smooth(data = subset(FACSdata_Good, Timepoint == timepoint), 
                                                   method = "lm", 
                                                   aes(fill = Timepoint), 
                                                   se = TRUE, 
                                                   alpha = 0.25, 
            ...
            # Save the plot for "Good" samples
            ggsave(filename = file.path(output_folder, paste0("LinReg_Good_", cells_colname, ".png")), plot = p_Good)
        }
        ...
        if (nrow(FACSdata_Bad) > 0) {
            rows_Bad <- calculate_linreg_by_timepoint(FACSdata_Bad, cells_colname)
            rows_Bad$Focus <- "Bad"  # Add the Recovery label for Bad samples
            Age_correlation_FACS <- rbind(Age_correlation_FACS, rows_Bad)
            
            # Plot regression for "Bad" samples with custom colors, size, and theme
            p_Bad <- ggplot(FACSdata_Bad, aes(x = Age, y = .data[[cells_colname]], color = Timepoint)) +
                geom_point(size = 2) +
                geom_smooth(method = "lm", aes(color = Timepoint), se = FALSE, size = 1) +  # Regression line in corresponding color
                ggtitle(paste("Linear Regression - Bad Recovery (", cells_colname, ")", sep = "")) +
                xlab("Age") +
                ylab(cells_colname) +
                theme(text = element_text(size = 14)) +
                theme(
            ...
            # Add confidence interval only if p-value is <= 0.05
            for (timepoint in unique(FACSdata_Bad$Timepoint)) {
                if (rows_Bad$p.value[rows_Bad$Timepoint == timepoint] <= 0.05) {
                    p_Bad <- p_Bad + geom_smooth(data = subset(FACSdata_Bad, Timepoint == timepoint), 
                                                       method = "lm", 
                                                       aes(fill = Timepoint), 
                                                       se = TRUE, 
                                                       alpha = 0.25, 
        ...
        # Save the plot for "Bad" samples
        ggsave(filename = file.path(output_folder, paste0("LinReg_Bad_", cells_colname, ".png")), plot = p_Bad)
    }
}
...
# for Gene expr.
metadataP_Gene <- metadataP_Gene %>% filter(SampleID %in% unique(data_rE_mean_matched$SampleID))
