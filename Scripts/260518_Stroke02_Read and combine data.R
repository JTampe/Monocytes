...
# Create NIHSS_End column based on TP4 -------------------------
Metadata_NeuroTest$NIHSS_End <- NA

# Get TP4 NIHSS values
tp4_values <- Metadata_NeuroTest %>%
    filter(Timepoint == "TP4") %>%
    select(SampleID, NIHSS) %>%
    rename(NIHSS_TP4 = NIHSS)

# Join TP4 NIHSS values back to all timepoints
Metadata_NeuroTest <- Metadata_NeuroTest %>%
    left_join(tp4_values, by = "SampleID") %>%
    mutate(NIHSS_End = NIHSS_TP4) %>%
    select(-NIHSS_TP4)

# Compute NIHSS_Ratio for SampleIDs with both TP1 and TP4
NIHSS_ratios <- Metadata_NeuroTest %>%
    filter(Timepoint %in% c("TP1", "TP4")) %>%
    group_by(SampleID) %>%
    summarise(
        NIHSS_TP1 = NIHSS[Timepoint == "TP1"][1],
        NIHSS_TP4 = NIHSS[Timepoint == "TP4"][1],
        NIHSS_Ratio = if (!is.na(NIHSS_TP1) && NIHSS_TP1 != 0) {
            (NIHSS_TP1 - NIHSS_TP4) / NIHSS_TP1
        } else {
            NA
        },
        .groups = "drop"
    ) %>%
    select(SampleID, NIHSS_Ratio)

# Join NIHSS_Ratio back to Metadata_NeuroTest
Metadata_NeuroTest <- Metadata_NeuroTest %>%
    left_join(NIHSS_ratios, by = "SampleID")

# Compute NIHSS_Ratio for SampleIDs with both TP1 and TP4
NIHSS_diff <- Metadata_NeuroTest %>%
    filter(Timepoint %in% c("TP1", "TP4")) %>%
    group_by(SampleID) %>%
    summarise(
        NIHSS_TP1 = NIHSS[Timepoint == "TP1"][1],
        NIHSS_TP4 = NIHSS[Timepoint == "TP4"][1],
        NIHSS_Diff = if (!is.na(NIHSS_TP1) && NIHSS_TP1 != 0) {
            (NIHSS_TP1 - NIHSS_TP4)
        } else {
            NA
        },
    ...
    select(SampleID, NIHSS_Diff)

# Join NIHSS_Ratio back to Metadata_NeuroTest
Metadata_NeuroTest <- Metadata_NeuroTest %>%
    left_join(NIHSS_diff, by = "SampleID")
...
# FACS
FACSdata <- as.data.frame(read_excel("241008_StrokeControlMFI_combined.xls", 
...
# add the metadata of NIHSS score of Patients
...
dataFINALmean <- dataFINAL %>%
    group_by(SampleID, Sex, Age, Category, Timepoint, Subpopulation, 
             Gene, GeneID, Gene_Group, Recovery, NIHSS, mRS, Barthel, MoCA,
             `HADS_Anxiety`, `HADS_Depression`, NIHSS_End, NIHSS_Ratio) %>%
...
#write_xlsx(data_mean_matched, "data_mean_matched.xlsx")
summary(dataFINAL)
