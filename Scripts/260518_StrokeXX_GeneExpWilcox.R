...
#### Recovery vs No Recovery (Mean ± SEM)* -------------------------------------
    ...
    for (j in 1:length(unique(df_gene$Subpopulation))) {
        ...
        tryCatch({
            ...
            ggline(subset(df_subpop_clean, !is.na(Recovery)), 
                ...
                labs(y = paste(gene, "expression (Z-scored from CT - CT Sample Median)"), 
                     title = title_facet) +
                scale_color_manual(values = c("Recovery" = "#4CAF50", "No recovery" = "#700606")) +
                theme_bw() +  # Apply white background
                theme(panel.grid.major = element_line(size = 0.2, linetype = 'solid', color = "gray80"), 
                      panel.grid.minor = element_blank(), 
                      panel.border = element_blank(),
            ...
            timepoints <- c("TP0", "TP1", "TP2", "TP3", "TP4")
            for (tp in timepoints) {
                good_values <- df_subpop_clean$mean_capZ[df_subpop_clean$Timepoint == tp & df_subpop_clean$Recovery == "Recovery"]
                bad_values <- df_subpop_clean$mean_capZ[df_subpop_clean$Timepoint == tp & df_subpop_clean$Recovery == "No recovery"]
                
                # Perform Wilcoxon test for the comparison of Recovery vs No recovery Recovery at each timepoint
            ...
            # Add results for Male vs Female for all timepoints as a single row
            results <- rbind(results, data.frame(WilcoxComparison = "Recovery vs No Recovery", Gene = gene, Subpopulation = sup, 
                                                 TP0_P_Value = p_values_list[["TP0"]],
                                                 TP1_P_Value = p_values_list[["TP1"]],
                                                 TP2_P_Value = p_values_list[["TP2"]],
                                                 TP3_P_Value = p_values_list[["TP3"]],
