...
today <- Sys.Date()
output_location <- paste(today,"_Control_wProteomics_Results", sep="")
...
raw_data <- read_csv("Combined Chip Run_mFlagFailed.csv", 
    skip = 11)
...
#relable Male & Female
metadata$Sex[grep("man",metadata$Sex)]  <- "Male"
metadata$Sex[grep("kvinna",metadata$Sex)]  <- "Female"
...
setwd("/Users/ju5263ta/Github/Monocytes/Data/Proteomics")
getwd ()

Proteomicsdata <- read_excel("For_correlation_markers.xls", 
...
Proteomicsdata <- as.data.frame(Proteomicsdata)

Proteomicsdata <- Proteomicsdata %>%
    rename(
        `01_Protein_CD36` = Protein_CD36,
        `02_Protein_CD169` = Protein_SIGLEC1,
        `03_Protein_CD86` = Protein_CD86,
        `04_Protein_CD64` = Protein_FCGR1A,
        `05_Protein_S100A9` = Protein_S100A9,
        `06_Protein_S100A8` = Protein_S100A8,
        `07_Protein_TLR8` = Protein_TLR8,
        `08_Protein_ANXA1` = Protein_ANXA1,
        `09_Protein_TSPO` = Protein_TSPO,
        `10_Protein_CX3CR1` = Protein_CX3CR1,
        `11_Protein_TGFB1` = Protein_TGFB1,
        `12_Protein_CD31` = Protein_PECAM1,
        `13_Protein_STAT6` = Protein_STAT6,
        `14_Protein_SIGLEC10` = Protein_SIGLEC10,
        `15_Protein_CD33` = Protein_CD33,
        `16_Protein_CD91` = Protein_LRP1,
        `17_Protein_TGM2` = Protein_TGM2,
        `18_Protein_CD38` = Protein_CD38,
        `19_Protein_CD11C` = Protein_ITGAX,
        `20_Protein_CD11B` = Protein_ITGAM
    )
...
# FACSdata distributions:-----------------------------------------
for (i in 7:19) {
    #values <- FACSdata[,i]
    cells <- colnames(FACSdata[i])
plot_FACS <- ggplot(FACSdata, aes(x = FACSdata[,i], fill = Sex)) +
    geom_histogram(position = "identity", alpha = 0.7, bins = 30) +
    labs(title = paste(cells," Distribution", x = cells, y = "Frequency")) +
    scale_fill_manual(values = c("#1D04C2","#A67C00")) +
    facet_wrap(~Sex, scales = "free_y")+
    theme(text = element_text(size = 14)) +
    theme(
...
# Save the plots as .png files
titel <- paste("Statistics_Ttest/histogram_FACS_",cells,"_distribution.png")
ggsave(titel, plot_FACS, width = 8, height = 6, units = "in")

}
## LinReg FACS outliers-----------------------------------------
createFolder("Statistics_LinReg")

Age_correlation_FACS <- data.frame(Cells = character(), Sex = character(), N = numeric(), p.value = numeric(), Coefficient = numeric(), Down95= numeric(), Up95= numeric(), Intercept = numeric())
...
Sex_correlation_FACS <- data.frame(Cells = character(), Sex = character(), N= numeric(), p.value = numeric(), Coefficient = numeric(), Down95= numeric(), Up95= numeric(), Intercept = numeric())
...
for (i in 7:19) {
      ...
      row_f <- data.frame(Cells = colnames(FACS_female[i]),  
        ...
        # add intercept to the results
        intercept_f <- summary(lm_result_f)$coefficients[1, 1]
        row_f$Intercept <- intercept_f
...
for (i in 7:19) {      
      ...
      # Linear regression to get the coefficient and its CI
        ...
        # add intercept to the results
        intercept_m <- summary(lm_result_m)$coefficients[1, 1]
        row_m$Intercept <- intercept_m
...
Age_correlation_rE <- data.frame(Subpopulation = character(), Gene = character(), GeneID = numeric(), Gene_Group = character(), N = numeric(), p.value = numeric(), Estimate = numeric(), Est_CI_Lower = numeric(), Est_CI_Upper = numeric(), Coefficient = numeric(), Coef_CI_Lower = numeric(), Coef_CI_Upper = numeric(), Intercept = numeric())
...
## LinReg Sex Outliers-----------------------------------------
# Pearson's Correlation uses linear relationship to correlate the data
Age_correlation_Sex_rE <- data.frame(Subpopulation = character(), Sex = character(), Gene = character(), GeneID = numeric(), Gene_Group = character(), N = numeric(), p.value = numeric(), Estimate = numeric(), Est_CI_Lower = numeric(), Est_CI_Upper = numeric(), Coefficient = numeric(), Coef_CI_Lower = numeric(), Coef_CI_Upper = numeric(), Intercept = numeric())
...
# Pearson's Correlation uses linear relationship to correlate the data
Age_correlation_rE_woOutliers <- data.frame(Subpopulation = character(), Gene = character(), GeneID = numeric(), Gene_Group = character(), N = numeric(), p.value = numeric(), Estimate = numeric(), Est_CI_Lower = numeric(), Est_CI_Upper = numeric(), Coefficient = numeric(), Coef_CI_Lower = numeric(), Coef_CI_Upper = numeric(), Intercept = numeric(), Fold = numeric(), Max_age = numeric(), Min_age = numeric())
...
for (i in 2:length(gene_list)) {
  ...
  for (j in 1:length(subtype_list)) {
    ...
    tryCatch({
      ...
      Max <- max(df_subpop$Age)
      Min <- min(df_subpop$Age)
...
# Pearson's Correlation uses linear relationship to correlate the data
Age_correlation_Sex_rE_woOutliers <- data.frame(Subpopulation = character(), Gene = character(), GeneID = numeric(), Gene_Group = character(), N = numeric(), p.value = numeric(), Estimate = numeric(), Est_CI_Lower = numeric(), Est_CI_Upper = numeric(), Coefficient = numeric(), Coef_CI_Lower = numeric(), Coef_CI_Upper = numeric(), Intercept = numeric(), Fold = numeric(), Max_age = numeric(), Min_age = numeric())
...
Age_correlation_FACS_woOutliers <- data.frame(Cells = character(), Sex = character(), N = numeric(), p.value = numeric(), Coefficient = numeric(), Down95= numeric(), Up95= numeric(), Intercept = numeric(), Fold = numeric(), Max_age = numeric(), Min_age = numeric())
...
Sex_correlation_FACS_woOutliers <- data.frame(Cells = character(), Sex = character(), N= numeric(), p.value = numeric(), Coefficient = numeric(), Down95= numeric(), Up95= numeric(), Intercept = numeric())
...
for (i in 7:19) {
      ...
      row_f <- data.frame(Cells = colnames(FACS_female_woOutliers[i]),  
        ...
        # calculate Fold induction
        Max_f <- max(FACS_female_woOutliers$Age)
        Min_f <- min(FACS_female_woOutliers$Age)
        FinalValue_f <- coef_x_f*Max_f+intercept_f
        InitialValue_f <- coef_x_f*Min_f+intercept_f
        Fold_f <- FinalValue_f/InitialValue_f
        row_f$Fold <- Fold_f
        row_f$Max_age <- Max_f
        row_f$Min_age <- Min_f
      ...
      # Linear regression to get the coefficient and its CI
        ...
        # calculate Fold induction
        Max_m <- max(FACS_male_woOutliers$Age)
        Min_m <- min(FACS_male_woOutliers$Age)
        FinalValue_m <- coef_x_m*Max_m+intercept_m
        InitialValue_m <- coef_x_m*Min_m+intercept_m
        Fold_m <- FinalValue_m/InitialValue_m
        row_m$Fold <- Fold_m
        row_m$Max_age <- Max_m
        row_m$Min_age <- Min_m
...
## LinReg Proteomics -----------------------------------------
createFolder("Statistics_LinReg_Proteomics")

Age_correlation_Proteomics <- data.frame(Protein = character(), Sex = character(), N = numeric(), p.value = numeric(), Coefficient = numeric(), Down95= numeric(), Up95= numeric(), Intercept = numeric())

for (i in 3:ncol(Proteomicsdata)) {
    x <- cor.test(Proteomicsdata[,i], Proteomicsdata$Age)
    row <- data.frame(Protein = colnames(Proteomicsdata[i]),
                      Sex = "All",
                      N=length(Proteomicsdata[,i]),
                      p.value = x$p.value,
                      Estimate = x$estimate,
                      Est_CI_Lower = x$conf.int[1],
                      Est_CI_Upper = x$conf.int[2],
    ...
    # Linear regression to get the coefficient and its CI
    lm_result <- lm(Proteomicsdata[,i] ~ Proteomicsdata$Age)
    coef_x <- summary(lm_result)$coefficients[2, 1]
    ci <- confint(lm_result)[2, ]
    ...
    Age_correlation_Proteomics <- rbind(Age_correlation_Proteomics, row)
    ...
    XProtein_woOutlier <- Proteomicsdata[-outliers,]
    if (nrow( XProtein_woOutlier) <= 0) {
        next
    }
    tryCatch({
        # repeat the same but without outliers!
        x <- cor.test(XProtein_woOutlier[,i], XProtein_woOutlier$Age)
        row <- data.frame(Protein = colnames(XProtein_woOutlier[i]),
                          Sex = "All",
                          N=length(XProtein_woOutlier[,i]),
                          p.value = x$p.value,
                          Estimate = x$estimate,
                          Est_CI_Lower = x$conf.int[1],
                          Est_CI_Upper = x$conf.int[2],
        ...
        # Linear regression to get the coefficient and its CI
        lm_result <- lm(XProtein_woOutlier[,i] ~ XProtein_woOutlier$Age)
        coef_x <- summary(lm_result)$coefficients[2, 1]
        ci <- confint(lm_result)[2, ]
...
Sex_correlation_Proteomics <- data.frame(Protein = character(), Sex = character(), N= numeric(), p.value = numeric(), Coefficient = numeric(), Down95= numeric(), Up95= numeric(), Intercept = numeric())

Proteomics_female <- Proteomicsdata %>% filter(Proteomicsdata$Gender == "Female")

for (i in 3:ncol(Proteomics_female)) {
    x_f <- cor.test(Proteomics_female[,i], Proteomics_female$Age)
    row_f <- data.frame(Protein = colnames(Proteomics_female[i]),  
                        Sex = "Female", 
                        N=length(Proteomics_female[,i]),
                        p.value = x_f$p.value,
                        Estimate = x_f$estimate,
                        Est_CI_Lower = x_f$conf.int[1],
                        Est_CI_Upper = x_f$conf.int[2],
    ...
    # Linear regression to get the coefficient and its CI
    lm_result_f <- lm(Proteomics_female[,i] ~ Proteomics_female$Age)
    coef_x_f <- summary(lm_result_f)$coefficients[2, 1]
    ci_f <- confint(lm_result_f)[2, ]
    ...
    Sex_correlation_Proteomics <- rbind(Sex_correlation_Proteomics, row_f)
    ...
    XProtein_woOutlier <- Proteomics_female[-outliers,]
    if (nrow( XProtein_woOutlier) <= 0) {
        next
    }
    tryCatch({
        # repeat the same but without outliers!
        x_f <- cor.test(XProtein_woOutlier[,i], XProtein_woOutlier$Age)
        row_f <- data.frame(Protein = colnames(XProtein_woOutlier[i]),  
                            Sex = "Female", 
                            N=length(XProtein_woOutlier[,i]),
                            p.value = x_f$p.value,
                            Estimate = x_f$estimate,
        ...
        # Linear regression to get the coefficient and its CI
        lm_result_f <- lm(XProtein_woOutlier[,i] ~ XProtein_woOutlier$Age)
        coef_x_f <- summary(lm_result_f)$coefficients[2, 1]
        ci_f <- confint(lm_result_f)[2, ]
...
Proteomics_male <- Proteomicsdata %>% filter(Proteomicsdata$Gender == "Male")
for (i in 3:ncol(Proteomics_male)) {      
    x_m <- cor.test(Proteomics_male[,i], Proteomics_male$Age)
    row_m <- data.frame(Protein = colnames(Proteomics_male[i]),   
                        Sex = "Male", 
                        N=length(Proteomics_male[,i]), 
                        p.value = x_m$p.value,
                        Estimate = x_m$estimate,
                        Est_CI_Lower = x_m$conf.int[1],
                        Est_CI_Upper = x_m$conf.int[2],
    ...
    # Linear regression to get the coefficient and its CI
    lm_result_m <- lm(Proteomics_male[,i] ~ Proteomics_male$Age)
    coef_x_m <- summary(lm_result_m)$coefficients[2, 1]
    ci_m <- confint(lm_result_m)[2, ]
    ...
    Sex_correlation_Proteomics <- rbind(Sex_correlation_Proteomics, row_m)
    ...
    tryCatch({
        # repeat the same but without outliers!
        x_m <- cor.test(XProtein_woOutlier[,i], XProtein_woOutlier$Age)
        row_m <- data.frame(Protein = colnames(XProtein_woOutlier[i]),  
                            Sex = "Female", 
                            N=length(XProtein_woOutlier[,i]),
                            p.value = x_m$p.value,
                            Estimate = x_m$estimate,
        ...
        # Linear regression to get the coefficient and its CI
        lm_result_m <- lm(XProtein_woOutlier[,i] ~ XProtein_woOutlier$Age)
        coef_x_m <- summary(lm_result_m)$coefficients[2, 1]
        ci_m <- confint(lm_result_m)[2, ]
...
Correlation_Proteomics <- rbind(Age_correlation_Proteomics, Sex_correlation_Proteomics)

file_name <- paste("Statistics_LinReg_Proteomics/", "Pearson's Correlation_Age_Proteomicsdata.csv", sep = "")
write.csv(Age_correlation_Proteomics, file_name, row.names = FALSE)
file_name <- paste("Statistics_LinReg_Proteomics/", "Pearson's Correlation_Sex_Proteomicsdata.csv", sep = "")
write.csv(Sex_correlation_Proteomics, file_name, row.names = FALSE)
file_name <- paste("Statistics_LinReg_Proteomics/", "Pearson's Correlation_Proteomicsdata.csv", sep = "")
write.csv(Correlation_Proteomics, file_name, row.names = FALSE)

## LinReg Proteomics NA-----------------------------------------
Proteomicsdata_NA <- Proteomicsdata
Proteomicsdata_NA[Proteomicsdata_NA == 0] <- NA

Age_correlation_Proteomics_NA <- data.frame(Protein = character(), Sex = character(), N = numeric(), p.value = numeric(), Coefficient = numeric(), Down95= numeric(), Up95= numeric(), Intercept = numeric())

for (i in 3:ncol(Proteomicsdata_NA)) {
    if (sum(is.na(Proteomicsdata_NA[,i]))>=30) {
        next
    }
    tryCatch({
    x <- cor.test(Proteomicsdata_NA[,i], Proteomicsdata_NA$Age)
    row <- data.frame(Protein = colnames(Proteomicsdata_NA[i]),
                      Sex = "All",
                      N=length(Proteomicsdata_NA[,i]),
                      p.value = x$p.value,
                      Estimate = x$estimate,
                      Est_CI_Lower = x$conf.int[1],
                      Est_CI_Upper = x$conf.int[2],
    ...
    # Linear regression to get the coefficient and its CI
    lm_result <- lm(Proteomicsdata_NA[,i] ~ Proteomicsdata_NA$Age)
    coef_x <- summary(lm_result)$coefficients[2, 1]
    ci <- confint(lm_result)[2, ]
    ...
    Age_correlation_Proteomics_NA <- rbind(Age_correlation_Proteomics_NA, row)
    ...
    XProtein_woOutlier <- Proteomicsdata_NA[-outliers,]
    if (nrow( XProtein_woOutlier) <= 0) {
        next
    }
    tryCatch({
...
Sex_correlation_Proteomics_NA <- data.frame(Protein = character(), Sex = character(), N= numeric(), p.value = numeric(), Coefficient = numeric(), Down95= numeric(), Up95= numeric(), Intercept = numeric())

Proteomics_female <- Proteomicsdata_NA %>% filter(Proteomicsdata_NA$Gender == "Female")

for (i in 3:ncol(Proteomics_female)) {
    if (sum(is.na(Proteomics_female[,i]))>=13) {
        next
    }
    tryCatch({
    x_f <- cor.test(Proteomics_female[,i], Proteomics_female$Age)
    ...
    Sex_correlation_Proteomics_NA <- rbind(Sex_correlation_Proteomics_NA, row_f)
    ...
    XProtein_woOutlier <- Proteomics_female[-outliers_f,]
    if (nrow( XProtein_woOutlier) <= 0) {
        next
    }
    tryCatch({
        ...
        if (x_f$p.value<=0.05){
            generateLinRegPlot_Sex(XProtein_woOutlier, "Age", "Protein_TGFB1", "TGFb1 Protein Exression (f)", "Plots_LinReg_Proteomics/", "Plots_LinReg_Protein_TGFB1")
        }
    } )
    } )
}

Proteomics_male <- Proteomicsdata_NA %>% filter(Proteomicsdata_NA$Gender == "Male")
for (i in 3:ncol(Proteomics_male)) {   
    if (sum(is.na(Proteomics_male[,i]))>=18) {
        next
    }
    tryCatch({
    x_m <- cor.test(Proteomics_male[,i], Proteomics_male$Age)
    ...
    Sex_correlation_Proteomics_NA <- rbind(Sex_correlation_Proteomics_NA, row_m)
    
    # check for outliners
    residuals_m <- lm_result_m$residuals # Extract residuals
    residual_sd_m <- sd(residuals_m) # Calculate standard deviation of residuals
    outlier_threshold_m <- 3 * residual_sd_m # Define a threshold for outliers (e.g., 3x SD)
    outliers_m <- which(abs(residuals_m) > outlier_threshold)  # Identify outliers
    
    XProtein_woOutlier <- Proteomics_female[-outliers_m,]
    if (nrow( XProtein_woOutlier) <= 0) {
        next
    }
    tryCatch({
...
Correlation_Proteomics_NA <- rbind(Age_correlation_Proteomics_NA, Sex_correlation_Proteomics_NA)

file_name <- paste("Statistics_LinReg_Proteomics/", "Pearson's Correlation_Age_Proteomics_NA.csv", sep = "")
write.csv(Age_correlation_Proteomics_NA, file_name, row.names = FALSE)
file_name <- paste("Statistics_LinReg_Proteomics/", "Pearson's Correlation_Sex_Proteomics_NA.csv", sep = "")
write.csv(Sex_correlation_Proteomics_NA, file_name, row.names = FALSE)
file_name <- paste("Statistics_LinReg_Proteomics/", "Pearson's Correlation_Proteomics_NA.csv", sep = "")
write.csv(Correlation_Proteomics_NA, file_name, row.names = FALSE)

## Make matrix for Proteomics
Sex_correlation_Proteomics_Matrix_est <- dcast(Sex_correlation_Proteomics, Protein ~ Sex, value.var = "Estimate", fun.aggregate = mean)
write.table(Sex_correlation_Proteomics_Matrix_est, "Statistics_LinReg_Proteomics/Sex_correlation_Proteomics_Matrix_est.txt", sep="\t", row.names=FALSE)

Sex_correlation_Proteomics_NA_Matrix_est <- dcast(Sex_correlation_Proteomics_NA, Protein ~ Sex, value.var = "Estimate", fun.aggregate = mean)
write.table(Sex_correlation_Proteomics_NA_Matrix_est, "Statistics_LinReg_Proteomics/Sex_correlation_Proteomics_NA_Matrix_est.txt", sep="\t", row.names=FALSE)



col_fun = colorRamp2(c(-1, -0.5, 0, 0.5, 1), c("darkblue","blue", "white", "red", "darkred"))

ggplot(df, aes(Protein, variable, fill = value)) + 
    geom_tile(color = "white", size = 1.5) +
    scale_fill_gradientn(colours = col_fun(c(-1, -0.5, 0, 0.5, 1)),
...
# Make the Matrix for the Correlation Coefficient (r):
...
# Make the Matrix for the Fold induction (only done for woOutliers):
Age_correlation_rE_Matrix_fold <- dcast(Age_correlation_rE_woOutliers, Gene_Order ~ Subpopulation, value.var = "Fold")
Age_correlation_Sex_rE_Matrix_fold <- dcast(Age_correlation_Sex_rE_woOutliers, Gene_Order ~ Info, value.var = "Fold")

write.table(Age_correlation_rE_Matrix_fold, "Statistics_LinReg/Age_Pearson-Fold_rE_Matrix_woOutliers.txt", sep="\t", row.names=FALSE)
write.table(Age_correlation_Sex_rE_Matrix_fold, "Statistics_LinReg/Age_Pearson-Fold_Sex_rE_Matrix_woOutliers.txt", sep="\t", row.names=FALSE)
