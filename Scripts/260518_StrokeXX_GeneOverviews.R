...
Gene_Overview_NIHSS <- LinReg_NIHSS_capZ %>%
    filter(p.value <= 0.05) %>% 
    filter(Subgroup == "All") %>% 
    group_by(Subpopulation, Gene)%>%
    summarise(NIHSS_Correlation = mean(Estimate, na.rm = TRUE))

Gene_Overview_NIHSS_End <- LinReg_NIHSS_End_capZ %>%
    filter(p.value <= 0.05) %>%  
    filter(Subgroup == "All") %>%
    group_by(Subpopulation, Gene)%>%
    summarise(NIHSS_End_Correlation = mean(Estimate, na.rm = TRUE))

Gene_Overview_NIHSS_Diff <- LinReg_NIHSS_Diff_capZ %>%
    filter(p.value <= 0.05) %>%  
    filter(Subgroup == "All") %>%
    group_by(Subpopulation, Gene)%>%
    summarise(NIHSS_Diff_Correlation = mean(Estimate, na.rm = TRUE))

Gene_Overview_NIHSS_Ratio <- LinReg_NIHSS_Ratio_capZ %>%
    filter(p.value <= 0.05) %>%  
    filter(Subgroup == "All") %>%
    group_by(Subpopulation, Gene)%>%
    summarise(NIHSS_Ratio_Correlation = mean(Estimate, na.rm = TRUE))
...
Gene_Overview_woTP <- merge(Gene_Overview_NIHSS,
                            Gene_Overview_NIHSS_End,
                            by = c("Gene", "Subpopulation"), 
                            all = TRUE)
Gene_Overview_woTP <- merge(Gene_Overview_woTP,
                            Gene_Overview_NIHSS_Diff, 
                            by = c("Gene", "Subpopulation"), 
                            all = TRUE)
Gene_Overview_woTP <- merge(Gene_Overview_woTP,
                            Gene_Overview_NIHSS_Ratio, 
                            by = c("Gene", "Subpopulation"), 
                            all = TRUE)
...
Gene_Overview_NIHSS <- LinReg_NIHSS_capZ %>%
    ...
    summarise(NIHSS_Correlation = Estimate)
Gene_Overview_NIHSS_End <- LinReg_NIHSS_End_capZ %>%
    filter(p.value <= 0.05) %>%  
    filter(Subgroup == "All") %>%
    group_by(Subpopulation, Gene, Timepoint) %>%
    summarise(NIHSS_End_Correlation = Estimate)
Gene_Overview_NIHSS_Diff <- LinReg_NIHSS_Diff_capZ %>%
    filter(p.value <= 0.05) %>%  
    filter(Subgroup == "All") %>%
    group_by(Subpopulation, Gene, Timepoint) %>%
    summarise(NIHSS_Diff_Correlation = Estimate)
Gene_Overview_NIHSS_Ratio <- LinReg_NIHSS_Ratio_capZ %>%
    filter(p.value <= 0.05) %>%  
    filter(Subgroup == "All") %>%
    group_by(Subpopulation, Gene, Timepoint) %>%
    summarise(NIHSS_Ratio_Correlation = Estimate)
...
Gene_Overview <- merge(Gene_Overview_NIHSS,
                       Gene_Overview_NIHSS_End, 
...
Gene_Overview_woTP_arrows <- Gene_Overview_woTP %>%
    mutate(across(c(NIHSS_Correlation, NIHSS_End_Correlation, NIHSS_Diff_Correlation, NIHSS_Ratio_Correlation, 
